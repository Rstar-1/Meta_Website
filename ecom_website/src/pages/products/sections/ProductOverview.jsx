              ...
              <div>
                ...
                <div className='grid-cols-5 gap-9 mt-12'>
                  {galleryImages.map((img, idx) => (
                    ...
                    >
                      <Image src={img} alt={`Thumbnail ${idx + 1}`} className='w-full h-100px flex object-contain' />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className='bg-white border border-ec p-20 rounded-5 mt-12'>
                  ...
                  <div className='bg-forth p-16 rounded-5 border border-ec'>
                    ...
                    <div className='flex flex-column gap-8'>
                      <div className='flex justify-between pb-6 bordb'>
                        ...
                        <span className='mini-text text-dark font-600'>{productData.brand}</span>
                      </div>
                      <div className='flex justify-between pb-6 bordb'>
                        <span className='mini-text text-gray'>Model</span>
                        <span className='mini-text text-dark font-600'>{productData.specs.find(s => s.label === 'Model')?.value || 'N/A'}</span>
                      </div>
                      <div className='flex justify-between pb-6 bordb'>
                        <span className='mini-text text-gray'>Color</span>
                        <span className='mini-text text-dark font-600'>{productData.specs.find(s => s.label === 'Color')?.value || 'N/A'}</span>
                      </div>
                      <div className='flex justify-between pb-6 bordb'>
                        <span className='mini-text text-gray'>Cartridge Type</span>
                        <span className='mini-text text-dark font-600'>{productData.specs.find(s => s.label === 'Cartridge Type')?.value || 'N/A'}</span>
                      </div>
                      <div className='flex justify-between pb-6 bordb'>
                        <span className='mini-text text-gray'>Page Yield</span>
                        <span className='mini-text text-dark font-600'>{productData.specs.find(s => s.label === 'Page Yield')?.value || 'N/A'}</span>
                      </div>
                      <div className='flex justify-between'>
                        <span className='mini-text text-gray'>Warranty</span>
                        <span className='mini-text text-dark font-600'>{productData.specs.find(s => s.label === 'Warranty')?.value || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {activeTab === 'compatibility' && (
                <div className='small-text text-dark'>
                  <p className='font-600 mb-10'>This product is compatible with the following models:</p>
                  <ul className='pl-20 m-0 flex flex-column gap-6 text-gray mini-text'>
                    {productData.specs.find(s => s.label === 'Compatibility') ? (
                      productData.specs.find(s => s.label === 'Compatibility').value.split(',').map((model, index) => (
                        <li key={index}>{model.trim()}</li>
                      ))
                    ) : (
                      <li>Standard Industry Equipment</li>
                    )}
                  </ul>
                </div>
              )}
