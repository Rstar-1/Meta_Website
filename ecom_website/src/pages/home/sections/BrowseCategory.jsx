  ...
  const services = [
    {
      ...
      desc: "We care success relationships fuel real success We love building brands and helping"
    },
    {
      id: 2,
  ...
  return (
    <Container>
      ...
      <div className='py-60'>
        ...
        <div className="grid-cols-3 md-grid-cols-2 sm-grid-cols-1 gap-12 mt-40">
          {services.map((item) => (
            <div key={item.id} className="service-card p-28 rounded-10">
              <div>
                <div className="service-icon-bg mb-25">
                  <Image src={item.icon} alt={item.title} width='30px' height='30px' className="flex object-contain" />
                </div>
                <h3 className="text-dark title-text font-600">{item.title}</h3>
                <p className="text-gray small-text mt-10">{item.desc}</p>
                <p className="text-dark small-text font-600 mt-20 flex items-center gap-6">Read More <Icon name="ArrowRight" width="14" height="14" stroke="currentColor" strokeWidth="2.5" /></p>
              </div>
            </div>
          ))}
        </div>
